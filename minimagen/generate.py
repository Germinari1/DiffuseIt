...
def _create_directory(dir_path):
    ...
    if not os.path.exists(img_path):
        ...
        raise FileExistsError(f"The directory {os.path.join(original_dir, img_path)} exists already and is not empty")
...
def load_minimagen(directory):
    ...
    if files != []:
        ...
        subdir = "tmp"
        print(f"\n\"state_dicts\" folder in {directory} is empty, so switched to using the most recent checkpoint. Source: \"tmp\".\n")
        files = os.listdir(os.path.join(directory, f"{subdir}"))

        if files == []:
            raise ValueError(f"Both \"/state_dicts\" and \"/tmp\" in {directory} empty. Perform model training to acquire state dictionaries for inference. ")
...
def sample_and_save(captions: list,
    ...
    assert not (minimagen is None and training_directory is None), \
        "Must supply: either training directory or MinImagen instance."
