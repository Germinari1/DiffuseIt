...
class Imagen(nn.Module):
    ...
    def __init__(
        ...
        :param cond_drop_prob: Probability of dropping for `classifier-free guidance 
        :param loss_type: Type of loss function to use. L1 (:code:`l1`), L2 (:code:`l2`), or Huber (:code:`huber`).
        :param lowres_sample_noise_level: Noise scale for `low-res conditioning augmentation 
        ...
        :param auto_normalize_img: Whether to auto normalize images to the range [-1., 1.]. Leave :code:`True` if
            ...
            before feeding in.
        :param dynamic_thresholding_percentile: Percentile value at which to activate `dynamic thresholding 
        ...
        assert num_unets == len(
            image_sizes), f'correct number of u-nets ({len(self.unets)}) for resolutions NOT supplied' \
                          f' {image_sizes}'
    ...
    def _p_sample_loop(self,
        ...
        """
        Given a Unet, iteratively generates a sample via reverse-diffusion
    ...
    def sample(
        ...
        :param cond_scale: Conditioning scale for `classifier-free guidance 
        :param lowres_sample_noise_level: Noise scale for `low-res noise conditioning augmentation 
        ...
        assert exists(text_embeds), 'Note: text or text encodings should be passed into Imagen'
        assert not (exists(text_embeds) and text_embeds.shape[
            -1] != self.text_embed_dim), f'invalid text embedding dimension. (should be {self.text_embed_dim})'
    ...
    def _p_losses(self,
        ...
        :param lowres_cond_img: Low-resolution version of images to condition on for super-resolution models.
            Shape (b, c, s, s)
        :param lowres_aug_times: Timesteps for low-resolution noise augmentation
    ...
    def forward(
        ...
        assert not (len(self.unets) > 1 and not exists(unet_number)), \
            f'specify which unet you want trained, from a range of 1 to {len(self.unets)}, ' \
        ...
        if exists(texts) and not exists(text_embeds):
            assert len(texts) == len(images), \
                'number of text captions does not match up with number of given images'
        ...
        assert not (exists(text_embeds) and text_embeds.shape[-1] != self.text_embed_dim), \
            f'invalid text embedding dimension: (should be {self.text_embed_dim})'
